...
1. **`start.bat` 더블 클릭**
2. 실행 시 시스템이 알아서 `Node.js` 구동 환경을 확인하고 없다면 즉시 백그라운드로 다운로드(1~2초)합니다.
3. 포트와 좀비 프로세스를 모두 초기화한 뒤 서버가 완벽하게 구동되며, Windows 내장 SSH를 통해 `localhost.run` 공용 인터넷 주소가 즉시 발급됩니다. (별도 터널링 프로그램 다운로드 불필요)
...
- **방문객 스마트폰 업로드**: `http://localhost:3000/upload.html` (`localhost.run`을 통해 모바일용 외부 도메인과 자동 연결됩니다)
...
- **초고속 Public Tunnel (localhost.run) 네이티브 통합**: 잦은 DNS 차단을 유발하던 Cloudflare를 걷어내고, Windows 자체 내장 SSH를 활용한 `localhost.run` 무료 터널링으로 교체했습니다. 외부 프로그램 다운로드조차 필요 없이 즉시 외부망(스마트폰) 접속 주소가 발급됩니다!
- **Node.js 자동 확보 로직 탑재**: 구동 환경에 프로그램이 아예 없는 극한의 PC 환경이더라도, 실행 시 1초 만에 최적화된 포터블 Node.js(v20.15.1)를 자동으로 감지/다운로드하여 완벽한 무설치 호환성을 보장합니다.
