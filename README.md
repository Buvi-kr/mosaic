...
### 2-8. ⚡ 사진 업로드 즉시 QR 개방 (조기 개방) 및 잠수 방지(AFK Timeout) 듀얼 파이프라인
전시장 대기열의 회전율을 극한으로 끌어올리기 위해 **"업로드 감지 즉시 조기 개방"**과 **"잠수 방지 안전 폴백"**이 결합된 이원화 듀얼 파이프라인이 작동합니다:

1. **⚡ 관람객 업로드 감지 즉시 QR 조기 개방 (Early Gate Open)**:
   - 관람객이 QR을 스캔하고 스마트폰에서 사진을 선택/촬영한 뒤 **[업로드(전송)] 버튼을 누르면**, 서버가 사진 파일(`POST /api/upload`)을 수신하는 찰나(0.01초)에 `sessionManager.openGateEarly('photo_uploaded')`를 실행합니다.
   - 백그라운드에서 워커 스레드가 모자이크를 합성/렌더링하는 동안, 키오스크 화면에는 **그 즉시 다음 대기자를 위한 새로운 QR 코드**가 생성되어 열립니다.
   - 따라서 빠른 관람객이 7초~10초 만에 사진을 전송하면 20초를 기다릴 필요 없이 즉각 다음 관람객이 QR을 찍고 체험을 시작할 수 있습니다.

2. **⏱️ 관람객 방치/잠수 방지 폴백 타이머 (AFK Safe Timeout - `visitorGateTimeout`)**:
   - 관람객이 QR만 찍어놓고 사진을 찍지 않거나 스마트폰을 든 채 멍하니 서 있는 경우를 방지합니다.
   - 관리자 패널(`admin.html`)에서 설정한 시간(기본값 **20초**, 슬라이더 조절 범위 **10초 ~ 120초**)이 지나면, 아직 사진이 올라오지 않았더라도 키오스크의 게이트를 자동으로 재개방(`isGateOpen: true`)하고 새 QR을 발급하여 전시 흐름이 멈추지 않도록 자동 복구합니다.
   - 관람객이 사진을 업로드하면 이 잠수 방지 폴백 타이머는 즉시 안전하게 취소(`clearTimeout`)됩니다.
...
| **⏱️ QR 자동 개방 / 잠수 방지** | `visitorGateTimeout` | 관람객이 QR 스캔 후 사진을 안 올리고 방치(잠수)할 때 새 QR을 자동 개방하는 시간 (기본 20초, 10~120초 조절). 사진 업로드 시에는 대기 없이 즉시 조기 개방됨. |
