...
sequenceDiagram
    ...
    participant S as 🖥️ 백엔드 서버 (워커 풀)
    participant D as 🪐 대형 디스플레이 (전시 큐)

    Note over A,K: [1. A의 즉시 촬영 & 전송]
    A->>K: QR 스캔 ➔ 즉시 카메라 오픈!
    K->>K: 게이트 일시 닫힘 (Busy 안내)
    A->>A: 셀카 촬영 & 크롭 후 [전송(업로드)] 클릭
    
    Note over A,K: [2. 사진 전송 즉시 0.01초 조기 개방]
    A->>S: 사진 업로드 수신 (0.01초)
    S->>K: ⚡ 키오스크 새 QR 즉시 자동 개방! (openGateEarly)
    
    Note over B,K: [3. B의 즉시 촬영 (대기 락 없음!)]
    B->>K: 새로 열린 QR 스캔 ➔ B 카메라 즉시 오픈!
    B->>B: 대기 화면 없이 B도 바로 셀카 촬영 시작!
    
    Note over A,D: [4. A의 1회차 전시 & 자유 결정]
    S->>D: new_mosaic (A 사진, 20초)
    D->>D: 🎬 A 사진 시네마틱 6.0배 줌 & 20초 카운트다운 전시
    A->>A: 모바일에서 결과 감상 (강제 마감 타이머 없음)
    
    par 멀티 워커 병렬 합성
        S->>S: A의 2회차 또는 B의 1회차 사진을 병렬로 동시 렌더링
    and 대형 디스플레이 쇼케이스 FIFO 대기열
        S->>D: new_mosaic (B 사진 완성)
        Note over D: 📥 A가 전시 중이면 B 사진은 큐(Queue)에 안전하게 적재!
        D->>D: A 전시 종료 즉시 B 사진 20초 시네마틱 단독 전시 시작!
    end
```

### 🎯 핵심 운영 메커니즘
1. **QR 스캔 즉시 촬영 (`isImmediate: true`)**:
   - 뒷사람 B는 지루한 "앞사람 촬영 중 대기" 화면을 볼 필요 없이, QR을 찍자마자 자기 폰에서 즉시 카메라가 열려 셀카를 찍습니다.
2. **1회차 후 고민 시간 무제한 (강제 타이머 제거)**:
   - 1회차 모자이크가 완성된 관람객은 대형 스크린을 여유롭게 감상하며 [한 번 더 찍기] 또는 [완료 및 다운로드]를 스스로 선택할 수 있습니다.
3. **쇼케이스 FIFO 큐 (`showcaseQueue`)**:
   - 앞사람 사진이 전시되는 도중 뒷사람 사진이 완성되어도 절대 가로채거나 덮어쓰지 않고, 도착 순서대로 20초씩 공평하게 전시됩니다.
4. **키오스크 QR 타이머 조율 (`visitorGateTimeout`)**:
   - 관리자가 설정한 시간(10~120초, 기본 20초)이 지나거나, 사진 업로드 감지 시 즉시 새 QR이 열려 회전율을 극대화합니다.
5. **무한 반복 방지 (키오스크 새 QR 재스캔 필수)**:
   - 체험을 마친 관람객이 스마트폰에서 무한정 사진을 올리는 것을 차단하며, 새 체험을 원할 경우 반드시 키오스크의 새 QR을 스캔해야 합니다.
