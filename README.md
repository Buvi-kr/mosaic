...
- **관람객 시야 방해 요소 0% (클린 아트워크 스크린)**:
  - 관람객이 보는 대형 디스플레이에는 어떠한 관리자 버튼이나 전체화면 버튼도 일체 노출되지 않으며, 순수 예술 작품과 실시간 인터랙션만 100% 렌더링.
  - 전시장 스태프를 위한 히든 단축키(키보드 `F` 키 or 빈 배경 더블클릭)만 백그라운드로 보존하여 필요 시 비상 제어 지원.
